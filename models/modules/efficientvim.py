...
class HSMSSD(nn.Module):
    def __init__(self, d_model, ssd_expand=1, A_init_range=(1, 16), state_dim=64):
        ...
        self.hz_proj = ConvLayer1D(d_model, d_model, 1, norm=None, act_layer=None)
        self.out_proj = ConvLayer1D(d_model, d_model, 1, norm=None, act_layer=None, bn_weight_init=0)
    ...
    def forward(self, x):
        ...
        h = self.hz_proj(h)
        h = self.out_proj(h * self.act(h) + h * self.D)
        y = h @ C
...
if __name__ == "__main__":
    ...
    # 创建测试输入张量
    x = torch.randn(1, 32, 28, 28).to(device)
    # 初始化 evim 模块
